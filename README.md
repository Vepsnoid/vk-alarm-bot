...
- **Повтор при сбое отправки**: если Max ответил сетевой ошибкой/5xx/429, пост остаётся в очереди (статус «В очереди (повтор)») и отправляется в следующих проверках; неверный токен или канал (4xx) повторять бессмысленно — такие посты сразу получают статус ошибки. После нескольких неудачных попыток пост помечается `failed` с причиной.
- **Всплеск публикаций**: за одну проверку обрабатывается до 200 новых постов на источник; если новостей больше, в журнал и колокольчик попадает предупреждение о пропущенных самых старых постах (уменьшите интервал проверки потока).
